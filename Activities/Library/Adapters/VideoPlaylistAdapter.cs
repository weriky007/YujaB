...
{
    public class VideoPlaylistAdapter : RecyclerView.Adapter, ListPreloader.IPreloadModelProvider
    {
            ...
            try
            {
                if (viewHolder is VideoPlaylistAdapterViewHolder holder)
                {
                    ...
                    if (item != null)
                    {
                        ...
                        GlideImageLoader.LoadImage(ActivityContext, item.Video.Owner?.OwnerClass?.Avatar, holder.ChannelImage, ImageStyle.CircleCrop, ImagePlaceholders.Drawable);
        ...
        private void ShowPopup(View v, DataVideoPlaylistObject item)
        {
            try
            {
                ...
                if (item.Video.IsOwner != null && item.Video.IsOwner.Value)
                {
                ...
                menuAddWatchLater.Click += (sender, args) =>
                {
                    try
                    {
                        LibrarySynchronizer.AddToWatchLater(item.Video);
                        popupWindow.Dismiss();
                    }
                    catch (Exception exception)
                    {
                ...
                menuAddPlaylist.Click += (sender, args) =>
                {
                    try
                    {
                        LibrarySynchronizer.AddToPlaylist(item.Video);
                        popupWindow.Dismiss();
                    }
                    catch (Exception exception)
                    {
                ...
                menuRemove.Click += (sender, args) =>
                {
                    try
                    {
                        ...
                        if (index != -1)
                        {
                            ...
                            if (data == null)
                            {
                                ListUtils.GlobalNotInterestedList.Add(item.Video);
                            }

                            var sqlEntity = new SqLiteDatabase();
                            sqlEntity.Insert_NotInterestedVideos(item.Video);
                            popupWindow.Dismiss();
                        }
                    }
                    catch (Exception exception)
                ...
                menuShare.Click += (sender, args) =>
                {
                    try
                    {
                        LibrarySynchronizer.ShareVideo(item.Video);
                        popupWindow.Dismiss();
                    }
                    catch (Exception exception)
                    {
                ...
                menuReport.Click += (sender, args) =>
                {
                    try
                    {
                        LibrarySynchronizer.AddReportVideo(item.Video);
                        popupWindow.Dismiss();
                    }
                    catch (Exception exception)
                    {
