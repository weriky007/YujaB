    ...
    public class InitPayPalPayment
    {
        ...
        private DropInRequest InitPayPal(string price, string payType)
        {
            try
            {
                ...
                var user = ListUtils.MyChannelList.FirstOrDefault();
                if (user != null)
                    threeDSecureRequest.InvokeEmail(user.Email).InvokeMobilePhoneNumber(user.PhoneNumber);
                ...
                PayPalRequest paypalRequest = new PayPalRequest(price)
                    ...
                    .InvokeBillingAgreementDescription(textPay)
                ...
                if (ListUtils.MySettingsList != null)
                {
